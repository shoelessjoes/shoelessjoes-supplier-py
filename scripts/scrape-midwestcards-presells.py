...
def main() -> None:
    ...
    parser.add_argument("--headed", action="store_true", help="Visible browser (Playwright launch mode only)")
    parser.add_argument("--manual-cf", action="store_true", help="Wait for manual Cloudflare (Playwright launch mode)")
    parser.add_argument("--browser-profile", default=str(DEFAULT_PROFILE_DIR))
    parser.add_argument("--channel", default="", help='Playwright launch only: "chrome" or "msedge"')
    parser.add_argument(
        "--cdp-url",
        default="",
        help=f"Attach to Chrome started by launch-chrome-for-midwest.ps1 (recommended). Default port: {DEFAULT_CDP_URL}",
    )
    parser.add_argument("--no-warmup", action="store_true", help="Skip homepage visit before scraping")
    parser.add_argument("--warmup-only", action="store_true", help="Homepage warmup then exit")
    ...
    if not cdp_url and not args.headed:
        print(
            "Use --cdp-url (recommended) or --headed.\n\n" + cdp_setup_instructions(profile_dir),
            file=sys.stderr,
        )
        sys.exit(1)

    manual_cf = (args.manual_cf or args.headed) and not cdp_url
    ...
    if cdp_url:
        print(f"Mode: CDP attach ({cdp_url})", file=sys.stderr)
    else:
        print(f"Mode: Playwright launch (profile: {profile_dir})", file=sys.stderr)
    ...
    with sync_playwright() as p:
        ...
        try:
            if not args.no_warmup:
                ...
                if "midwestcards.com" not in (page.url or ""):
                ...
                if not wait_past_cloudflare(page, cf_timeout_ms, manual=manual_cf, label="homepage"):
                    print(cdp_setup_instructions(profile_dir, cdp_url or DEFAULT_CDP_URL), file=sys.stderr)
                    sys.exit(1)
                if args.warmup_only:
